...
const AdminNotifications = () => {
  ...
  const fetchNotifications = async () => {
    try {
      const [damagedRes, lowStockRes] = await Promise.all([
        axios.get("http://localhost:8081/api/books/damaged"),
        axios.get("http://localhost:8081/api/books/low-stock")
      ]);
  ...
  const handleRepair = async (bookId, bookName, damagedCopies) => {
    ...
    try {
      await axios.post("http://localhost:8081/api/books/repair", null, {
